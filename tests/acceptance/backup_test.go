...
func TestBackupPublishesVerifiedArchivesForEveryMutableServiceVolume(t *testing.T) {
	...
	serviceNames := []string{"gluetun", "qbittorrent", "prowlarr", "sonarr", "radarr", "profilarr", "jellyfin", "seerr"}
	for _, serviceName := range serviceNames {
		volumeName := "media-staging_" + serviceName + "-config"
		volumeRoot := filepath.Join(fixtureRoot, volumeName)
...
func TestBackupFailureResumesServicesWithoutPublishingManifest(t *testing.T) {
	...
	configPath := backupConfig(t, repository, temporary)
	fixtureRoot := filepath.Join(temporary, "volumes")
	for _, serviceName := range []string{"gluetun", "qbittorrent", "prowlarr", "sonarr", "radarr", "profilarr", "jellyfin", "seerr"} {
		volumeRoot := filepath.Join(fixtureRoot, "media-staging_"+serviceName+"-config")
		if err := os.MkdirAll(volumeRoot, 0o700); err != nil {
			t.Fatalf("create %s failure fixture: %v", serviceName, err)
		}
		if err := os.WriteFile(filepath.Join(volumeRoot, "identity.txt"), []byte(serviceName+"\n"), 0o600); err != nil {
			t.Fatalf("write %s failure fixture: %v", serviceName, err)
		}
	}

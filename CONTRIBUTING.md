...
The local CLI must be statically linked. `doctor` bind-mounts its own executable into the pinned application images to run
disposable storage probes, and a binary linked against the host's C library may not start inside those containers.
...
2. Build the CLI locally with `CGO_ENABLED=0 go build -o bin/media-stack ./cmd/media-stack`.

...
enabled, set the Jellyfin administrator's local password, verify both login paths, and configure the selected Environment's
Radarr service as the default non-4K Movie Library request destination using the pinned Profilarr profile, shared root, and
automatic search. Repeated apply makes no policy writes after convergence.
...
administrator sign-in remains available. To prove the complete request path against real services, point at an already
applied, disposable Staging Environment and run:
...
selected archive into a temporary Docker volume with UID/GID ownership preserved and independently compares the recovered file metadata and contents. It then
...
| `media-stack plan --environment production|staging [--config path]` | Available, configuration observation | Reports ordered create, update, restart, guide, and retained-unknown observations as YAML comments, followed by rendered Compose YAML. It compares complete Compose service hashes; inventories project containers, networks, and volumes; and reuses the application reconciliation contracts behind a mutation-blocking HTTP boundary. If an observation dependency is unavailable, that subject is explicitly deferred. Use it before `apply` to inspect Declared Configuration drift or pipe the complete output to `docker compose config`. Saved Plan Artifacts remain a later planner phase. |
...
Unknown project containers, networks, and volumes are outside the Service Configuration Contract. `plan` reports them deterministically and `apply` retains them; they are observations rather than change actions. A plan containing only retained unknowns therefore prints `# plan: no changes` before the still-valid Compose document. Guarded pruning is implemented separately.

The qBittorrent, Radarr, Sonarr, Prowlarr, Profilarr, Jellyfin, and Seerr apply phases share their renderer and supported API reconciliation contracts with `plan`. Planning permits read-only calls, authentication, and upstream connection tests, but blocks application mutations and reports the attempted change instead. Profilarr-owned policy drift is reported as a guided action. Later planner phases will add saved Plan Artifacts, Environment locking, confirmation, and volatile safety rechecks. Planned `destroy` will never purge media, torrent payloads,
